...
namespace UFramework
{
    public class Promise : IPromise, IPendingPromise, IRejectable, IPromiseInfo
    {
        ...
        {
            this.Catchs(
                (SException exception) => { PropagateUnhandledException(this, exception); }
            );
        }
        ...
        public IPromise Then(Func<IPromise> onResolved, Action<SException> onRejected)
        {
            ...
            Action resolverHandler = () =>
            {
                if (onResolved != null)
                {
                    Action nextResolveHandler = () => { resultPromise.Resolve(); };
        ...
        public IPromise<ConvertedT> Then<ConvertedT>(Func<IPromise<ConvertedT>> onResolved,
            Action<SException> onRejected)
        {
            Promise<ConvertedT> resultPromise = new Promise<ConvertedT>();
            Action resolveHandler = () =>
            {
                onResolved()
                    .Then((ConvertedT chainedValue) => { resultPromise.Resolve(chainedValue); },
                        (SException exception) => { resultPromise.Reject(exception); });
            };
    ...
    public class Promise<PromisedT> : IPromise<PromisedT>, IPendingPromise<PromisedT>, IRejectable, IPromiseInfo
    {
        ...
        public Promise(Action<Action<PromisedT>, Action<SException>> resolver)
        {
            ...
            try
            {
                Action<PromisedT> resolveHandler = (PromisedT value) => { Resolve(value); };

                Action<SException> rejectHandler = (SException exception) => { Reject(exception); };
        ...
        private void ActionHandlers(IRejectable resultPromise, Action<PromisedT> resolveHandler,
            Action<SException> rejectHandler)
        {
            if (this.curState == PromiseState.Resolved)
            {
        ...
        public IPromise<PromisedT> Catchs(Action<SException> onRejected)
        {
            ...
            Action<PromisedT> resolveHandler = (PromisedT value) => { resultPromise.Resolve(value); };
        ...
        {
            this.Catchs(
                (SException exception) => { Promise.PropagateUnhandledException(this, exception); }
            );
        }
