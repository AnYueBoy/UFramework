...
public class ColoredHeaderGUI : EditorWindow
{
    ...
    {
        var scriptObject = MonoScript.FromScriptableObject(this);
        string scriptAssetPath = AssetDatabase.GetAssetPath(scriptObject);
        string directory = Path.GetDirectoryName(scriptAssetPath);
        int lastIndex = directory!.LastIndexOf(@"\");
        string parentDirectory = directory.Substring(0, lastIndex);

        return parentDirectory + @"\Settings.asset";
    }
    ...
    private static class Strings
    {
        ...
        public const string LOGO_TEXT = "ColoredHeaders";
    ...
    private static void CreateWindow()
    {
        ...
        window.minSize = new Vector2(325f, 390f);
    }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            // 检查代码块中的GUI是否产生了变动，不同于GUI.changed在任何GUI变动都会返回True，这一方法产生了类似作用域的范围
    ...
    {
        var path = EditorUtility.SaveFilePanelInProject(Strings.SAVE_PRESET_PANEL_TITLE, "New Header Preset", "asset",
            "Enter a file name.");
        if (path.Length <= 0)
        {
            Debug.LogError("Invalid operation.");
